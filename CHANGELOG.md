...
- **Polish round (Daniel's review):** nav segments paint their rounded pill correctly in every state (CTk "transparent" labels don't repaint when the parent fill changes — labels now get explicit backgrounds), and the end segments are inset past the outer pill's corner curve so nothing rectangular pokes through the border (screenshot-verified this time); segment↔border spacing is now a uniform 6px on every side (pill 40px/r20, segments 28px/r14 — the nesting math is in the code comment). New `docs/RUN-DEV.md`: how to run the dev version from source without installing. Nav round 3: fully-round capsules (corner radius 999) for segments AND the container, doubled inner padding (pill 50px, segments 34px, uniform 8px gaps — clearance math updated); theme/gear circles hug each other and the window edge; the model·hotkey·language status line moved OUT of the pages INTO the top bar next to the icons (compact format, auto-hides when the window is too narrow to fit beside the centered pill; tabs refresh it via ctx.refresh_status); default window 940×560. Round 4: segment icons sit closer to their labels; the global-settings line (model · hotkey · language · mode) moved into the WINDOW TITLE — "Eqho Dashboard — Distil Large v3 · Alt+Q · EN · toggle" — freeing the bar (collision auto-hide machinery removed); the nav is now RENDERED AS ONE SUPERSAMPLED PIL IMAGE (`_NavPill`) — widget nesting could never do tight capsule-in-capsule layouts, and raw canvas shapes had no anti-aliasing and mangled the font into a fake italic; the PIL bar gives a uniform 4px stroke gap on ALL sides, near-zero gaps between the buttons, true Inter-SemiBold + Phosphor rendering, smooth curves (3× downsampled), hover/click via hit-ranges. History: renders 40 entries per page with a "Show more" button (200 cards at once made the tab heavy to open) and gained the same top inset as the other sections; the top-right theme/gear buttons are small tight circles (28 px) instead of pills; pages no longer repeat their own section title — just a slim model·hotkey·language status line, then the options (embedded Overlay/About sections drop their inner labels too); **UI Zoom moved from General into Settings**, sitting beside Theme in a two-column Appearance row; **History gained a date filter** — a period dropdown (Today / Yesterday / Last 7 / Last 30 days) plus the search box now also matches date stamps, so typing `2026-07-14` finds a day directly.
