...
## CUDA / models
- **The model-switch native crash is SOLVED (v0.8.0) by process isolation.** Inference runs in a child process (`model_host.py`); switching = kill+respawn a fresh child (one model per process). The main app never loads a model. Do NOT reintroduce in-process WhisperModel loading. `run.py` MUST call `multiprocessing.freeze_support()` or the frozen exe spawns whole-app children. multiprocessing start method is "spawn" (Windows) — the child imports `src.model_host`; keep heavy imports (faster_whisper) inside the child load path, not at module top.
- **Model changes RESTART the app (v0.7.2, Option A).** `App.restart()` releases the single-instance socket, respawns the process, and `os._exit(0)`s so CUDA/VRAM fully frees; the new process loads the chosen model cleanly. UI model changes go through `ctx.change_model()` (one-time confirm dialog + `model_restart_notice` setting). Do NOT re-add in-process `reload_model()` for model switches. Language changes do NOT restart (passed per-transcribe).
